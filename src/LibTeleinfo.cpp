...
struct _ValueList ValuesTab[51];	//Allocate static table of 50 items
...
unsigned char TInfo::calcChecksum(char *etiquette, char *valeur) 
{
  ...
  if (etiquette && valeur) {
    ...
    if (strlen(etiquette) && strlen(valeur)) {
      while (*etiquette)
	if (isalnum(*etiquette) || *etiquette=='.') {
	        sum += *etiquette++ ;
	} else {
		return 0;
	}
...
ValueList * TInfo::checkLine(char * pline) 
{
  ...
  // Loop in buffer 
  while ( p < pend ) {
    // Check if first caracter is alphanum
    //if (!(isalnum(*p) || *p !=' ' || *p != )
    //  return NULL;
